...
exports.login = async (req, res) => {
  try {
    ...
    if (!user.isVerified) {
      ...
      return res
        .status(403)
        .json({ message: "Please verify your email or phone number" });
    }

    console.log("Stored password hash:", user.password);
    const isMatch = await user.comparePassword(password);
    console.log(
      "Password match result for user:",
      user.email || user.phone_number,
      "is:",
      isMatch
    );
...
exports.getAll = async (req, res) => {
  try {
    const users = await User.find().select(
      "-password -otp -resetOtp -accessToken"
    );
...
exports.getWorkSchedule = async (req, res) => {
  try {
    const user = await User.findById(req.params.id).select(
      "role dayOfWeek startTimeInDay endTimeInDay startDay endDay"
    );
    if (!user) return res.status(404).json({ message: "User not found" });
    if (user.role !== "doctor")
    ...
    res
      .status(500)
      .json({ message: "Server error while fetching work schedule" });
  }
};
...
exports.updateWorkSchedule = async (req, res) => {
  try {
    const { id } = req.params;
    const { dayOfWeek, startTimeInDay, endTimeInDay, startDay, endDay } =
      req.body;
    ...
    if (user.role !== "doctor") {
      return res
        .status(403)
        .json({ message: "Chỉ có thể cập nhật lịch cho bác sĩ." });
    }
...
exports.clearWorkSchedule = async (req, res) => {
  try {
    ...
    if (!currentUser || currentUser.role !== "admin") {
      return res
        .status(403)
        .json({ message: "Chỉ admin mới có quyền xóa lịch làm việc" });
    }
    ...
    res
      .status(500)
      .json({ message: "Server error while clearing work schedule" });
  }
};
...
exports.addCertification = async (req, res) => {
  try {
    ...
    if (currentUser.role !== "admin" && currentUser.id !== id) {
      return res
        .status(403)
        .json({ message: "You can only add certification for yourself" });
    }

    if (user.role !== "doctor") {
      return res
        .status(403)
        .json({ message: "Only doctor can have certifications" });
    }

    const { title, issuer, issueDate, expiryDate, description, fileUrl } =
      req.body;
    ...
    res.status(200).json({
      message: "Certification added successfully, waiting for admin approval",
...
exports.updateCertification = async (req, res) => {
  try {
    ...
    if (currentUser.role !== "admin" && currentUser.id !== id) {
      return res
        .status(403)
        .json({ message: "You can only update experience for yourself" });
    }

    if (user.role !== "doctor") {
      return res
        .status(403)
        .json({ message: "Only doctor can have experiences" });
    }
    ...
    if (cert.verified && currentUser.role !== "admin") {
      return res.status(403).json({
        message: "Cannot update verified experience unless you are an admin",
      });
    }
    ...
    res
      .status(500)
      .json({ message: "Server error while updating certification" });
  }
};
...
exports.deleteCertification = async (req, res) => {
  try {
    ...
    if (currentUser.role !== "admin" && currentUser.id !== id) {
      return res
        .status(403)
        .json({ message: "You can only delete certification for yourself" });
    }
...
exports.approveCertification = async (req, res) => {
  try {
    ...
    if (currentUser.role !== "admin") {
      return res
        .status(403)
        .json({ message: "Only admin can approve certifications" });
    }
    ...
    res
      .status(500)
      .json({ message: "Server error while approving certification" });
  }
};
...
exports.rejectCertification = async (req, res) => {
  try {
    ...
    if (currentUser.role !== "admin") {
      return res
        .status(403)
        .json({ message: "Only admin can reject certifications" });
    }
    ...
    res
      .status(500)
      .json({ message: "Server error while rejecting certification" });
  }
};
...
exports.addExperience = async (req, res) => {
  try {
    ...
    if (currentUser.role !== "admin" && currentUser.id !== id) {
      return res
        .status(403)
        .json({ message: "You can only add experience for yourself" });
    }
    ...
    res.status(200).json({
      message: "Experience added successfully, waiting for admin approval",
...
exports.deleteExperience = async (req, res) => {
  try {
    ...
    if (currentUser.role !== "admin" && currentUser.id !== id) {
      return res
        .status(403)
        .json({ message: "You can only delete experience for yourself" });
    }
...
exports.approveExperience = async (req, res) => {
  try {
    ...
    if (currentUser.role !== "admin") {
      return res
        .status(403)
        .json({ message: "Only admin can approve experiences" });
    }
    ...
    res
      .status(500)
      .json({ message: "Server error while approving experience" });
  }
};
...
exports.rejectExperience = async (req, res) => {
  try {
    ...
    if (currentUser.role !== "admin") {
      return res
        .status(403)
        .json({ message: "Only admin can reject experiences" });
    }
    ...
    res
      .status(500)
      .json({ message: "Server error while rejecting experience" });
  }
};
